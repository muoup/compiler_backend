...
ir::block::block_instruction parser::parse_branch(ir::parser::lex_iter_t &start, ir::parser::lex_iter_t end) {
    auto condition = parse_value(start, end);
    debug::assert(start++->value == "then", "Expected then");

    auto true_branch = start++->value;
    debug::assert(start++->value == "else", "Expected else");

    auto false_branch = start++->value;

    return ir::block::block_instruction {
        std::make_unique<ir::block::branch>(std::move(true_branch), std::move(false_branch)),
        std::vector<ir::value> { std::move(condition) }
    };
}
